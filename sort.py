...
if __name__ == "__main__":
    ...
    for shader in loaded_data["shaders"]:
        normalized_name = "".join([c if c.isalnum() else "_" for c in shader["info"]["name"]])
        shader_path = os.path.join(user_name, f'{shader["info"]["id"]}_{normalized_name}')
        ...
        for renderpass in shader["renderpass"]:
            ...
            with open(os.path.join(shader_path, f"{rp_name.replace(' ', '_')}.frag"), "w") as f:
                f.write(code)
